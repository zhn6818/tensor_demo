...
#define CHECK(status)                                                                                                  \
    ...
    {                                                                                                                  \
        ...
        if (ret != 0)                                                                                                  \
        {                                                                                                              \
            tensorrt_virgo_log::gLogError << "Cuda failure: " << ret << std::endl;                                                 \
            abort();                                                                                                   \
        }                                                                                                              \
    } while (0)
...
#define CHECK_RETURN_W_MSG(status, val, errMsg)                                                                        \
    ...
    {                                                                                                                  \
        if (!(status))                                                                                                 \
        {                                                                                                              \
            tensorrt_virgo_log::gLogError << errMsg << " Error in " << __FILE__ << ", function " << FN_NAME << "(), line " << __LINE__     \
                      << std::endl;                                                                                    \
            return val;                                                                                                \
        }                                                                                                              \
    } while (0)
...
#define ASSERT(condition)                                                   \
    ...
    {                                                                       \
        if (!(condition))                                                   \
        {                                                                   \
            tensorrt_virgo_log::gLogError << "Assertion failure: " << #condition << std::endl;  \
            abort();                                                        \
        }                                                                   \
    } while (0)
...
namespace tensorrt_virgo_common {
...
//! Return vector of indices that puts magnitudes of sequence in descending order.
    ...
    std::vector<std::string> classify(
        ...
        const auto inds = tensorrt_virgo_common::argMagnitudeSort(output.cbegin(), output.cend());
...
// Returns indices of highest K magnitudes in v.
    ...
    std::vector<size_t> topKMagnitudes(const std::vector<T> &v, const size_t k) {
        std::vector<size_t> indices = tensorrt_virgo_common::argMagnitudeSort(v.cbegin(), v.cend());
        indices.resize(k);
        return indices;
    }
...
// cases where scaling factors are asymmetric.
    ...
    inline void setDummyInt8DynamicRanges(const IBuilderConfig *c, INetworkDefinition *n) {
        ...
        if (c->getFlag(BuilderFlag::kINT8)) {
            tensorrt_virgo_log::gLogWarning
...
// New vPPM(variable sized PPM) class with variable dimensions.
    ...
    template<int C, int H, int W>
    void readPPMFile(const std::string &filename, tensorrt_virgo_common::PPM<C, H, W> &ppm) {
...
// Return m rounded up to nearest multiple of n
    ...
    inline void loadLibrary(const std::string &path) {
        ...
        if (handle == nullptr) {
#ifdef _MSC_VER
            tensorrt_virgo_log::gLogError << "Could not load plugin library: " << path << std::endl;
#else
            tensorrt_virgo_log::gLogError << "Could not load plugin library: " << path << ", due to: " << dlerror()
                                    << std::endl;
#endif
        }
    }
