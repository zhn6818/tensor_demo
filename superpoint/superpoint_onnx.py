...
def main():
    ...
    parser.add_argument('--weight_file', default="./superpoint/superpoint_v1.pth",
                        help="pytorch weight file (.pth)")
    parser.add_argument('--output_dir', default="superpoint", help="onnx model file output directory")
    ...
    data = torch.ones((1,1,256,256)).to(device)
    superpoint_model = superpoint_model.to(device)
    with torch.no_grad():
        result = superpoint_model(data)
